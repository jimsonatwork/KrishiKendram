...
describe('FarmsService', () => {
  ...
  it('creates a crop through the canonical FarmsService mutation boundary', async () => {
    const farm = {
      id: 'farm-1',
      ownerId: 'user-1',
    };
    ...
    expect(prisma.crop.findFirst).toHaveBeenCalledWith({
      where: {
        ...
        sowingDate: {
          gte: new Date('2026-09-18T00:00:00.000'),
          lt: new Date('2026-09-19T00:00:00.000'),
        },
      },
    });
    ...
    expect(
      ...
      prisma.crop.findFirst.mock.invocationCallOrder[0],
    );
  ...
  it('rejects a crop when the central Registry rejects the crop name', async () => {
    prisma.farm.findUnique.mockResolvedValue({
      id: 'farm-1',
      ownerId: 'user-1',
    });
  ...
  it('returns the existing same-day crop without creating a duplicate', async () => {
    const farm = {
      id: 'farm-1',
      ownerId: 'user-1',
    };

    const sowingDate = new Date('2026-09-18T14:00:00.000Z');
  ...
  it('does not validate or create a crop when authorization denies access', async () => {
    prisma.farm.findUnique.mockResolvedValue({
      id: 'farm-1',
      ownerId: 'owner-1',
    });
